...
# TODOPatch - a square patch of size [2 * patchL + 1, 2 * patchL + 1, 3]
# TODOMask - specifies which elements in TODOPatch are (0) filled and useful, or (1) empty and waiting to be filled in
# textureIm - The texture image is of size [texImRows, texImCols, 3]
# patchL - patch length
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	for r in range(ssd_rows):
		for c in range(ssd_cols):
			...
			# for all pixels where TODOMask = 0 (filled), and store the result in SSD
			...
			for rr in range(patch_rows):
				for cc in range(patch_cols):
					...
					if TODOMask[rr, cc] == 0:
						...
						diff = (TODOPatch[rr, cc] * 1.0) - (textureIm[r - rr, c - cc] * 1.0)
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	...
	for i in range(patchSize):
		for j in range(patchSize):
			...
			if TODOMask[i, j] == 1:
				imHole[iPatchCenter-patchL+i, jPatchCenter-patchL+j] = selectPatch[i, j]
...
# im = Image.open('slint.jpg').convert('RGB')
